...
void UART_Init(void){
  SYSCTL_RCGCUART_R |= 0x01;            // activate UART0---> change to UART1
  SYSCTL_RCGCGPIO_R |= 0x01;            // activate port [use PORTC in
...
#define NVIC_EN0_INT6   0x00000040  // Interrupt 6 enable
 // static implementation of class
// Initialize UART0
// Baud rate is 115200 bits/sec
// Lab 9
void UART_Init(void){
  ...
  UART1_IBRD_R = 43; 
// IBRD=int(80000000/(16*115,200)) = int(43.403)
  UART1_FBRD_R = 26;  
// FBRD = round(0.403 * 64) = 26
  ...
  GPIO_PORTC_AMSEL_R &= ~0x30; // No analog on PC4-5
	...
	UART1_IFLS_R&= 0xD7;
	UART1_IFLS_R|=0x10;
	NVIC_PRI1_R = (NVIC_PRI1_R&0xFF1FFFFF)|0x00800000;	//bits 21, 22, 23 will be set to priority level 4
...
void UART1_Handler(void){
  ...
  GPIO_PORTF_DATA_R  ^= 0x02; // triple toggle debugging
  GPIO_PORTF_DATA_R  ^= 0x02;
 // write this
	char letter;
	while((UART1_FR_R&UART_FR_RXFE) == 0){
		if(!RxFifo.IsFull())
    {letter = UART1_DR_R&0x00FF;
    RxFifo.Put(letter);}
